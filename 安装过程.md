...
```sh
chmod +x ./cri-dockerd
cp -rf ./cri-dockerd /usr/bin/

cp -rf ./cri-docker.s* /etc/systemd/system/
...
```sh
kubeadm config images pull --kubernetes-version v1.24.3 --image-repository registry.aliyuncs.com/google_containers --cri-socket unix:///var/run/cri-dockerd.sock
```
...
```sh
kubeadm config images list --kubernetes-version v1.24.3 --image-repository registry.aliyuncs.com/google_containers --cri-socket unix:///var/run/cri-dockerd.sock
```
